# 👋 Hi, I'm Paul Estrella

**Founder & AI Product Builder — LLM Integration**

I build AI-powered products from idea to production,
combining LLM integration, user experience, and backend systems.

---

## 🚀 Featured Project: EmotiNest

**AI-powered HealthTech platform for families with 
neurodivergent children. Live on iOS & Android.**

### What I built:
- Designed the full UX and daily interaction model
- Built EmotiCoach — an AI assistant using LLMs 
  for personalized guidance
- Developed backend logic and data handling with Supabase
- Shipped from idea to production as a mobile app

👉 https://www.myemotinest.com/

---

## 🛠️ Stack

### 🤖 AI
![Claude API](https://img.shields.io/badge/Claude%20API-D97757?style=flat-square)
![LLMs](https://img.shields.io/badge/LLMs-000000?style=flat-square)
![Prompt Engineering](https://img.shields.io/badge/Prompt%20Engineering-4B0082?style=flat-square)

### ⚙️ Backend
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-02569B?style=flat-square)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square)
![Python](https://img.shields.io/badge/Python-learning-3776AB?style=flat-square&logo=python&logoColor=white)

### 📱 Frontend
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)
![Web Apps](https://img.shields.io/badge/Web%20Apps-000000?style=flat-square)
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)

---

## 🎯 What I'm Building Next
- LLM-powered chatbots and AI assistants
- AI-powered web MVPs
- Products that solve real user problems with AI

---

## 📫 Connect
- LinkedIn: https://www.linkedin.com/in/paulestrellapm/
- Website: https://www.myemotinest.com/
